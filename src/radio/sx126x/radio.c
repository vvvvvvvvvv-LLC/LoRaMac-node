...
void RadioInit(RadioEvents_t *events)
{
    ...
    LOG_DBG("TxTimeoutTimer %p, RxTimeoutTimer %p", &TxTimeoutTimer, &RxTimeoutTimer);
...
void RadioSetChannel(uint32_t freq)
{
    LOG_ERR("RadioSetChannel %d", freq);
                      ...
                      bool iqInverted, bool rxContinuous)
{
    ...
    if (fixLen == true)
    {
        LOG_ERR("Setting MaxPayloadLength %d", payloadLen);
        MaxPayloadLength = payloadLen;
    }
    else
    {
        LOG_ERR("Setting MaxPayloadLength 0xFF");
    ...
    case MODEM_LORA:
        ...
        RadioSetModem((SX126x.ModulationParams.PacketType == PACKET_TYPE_GFSK) ? MODEM_FSK : MODEM_LORA);
        SX126xSetModulationParams(&SX126x.ModulationParams);
        LOG_ERR("E. CrcMode %d, HeaderType %d, InvertIQ %d, PayloadLength %d, PreambleLength %d",
                      ...
                      uint8_t hopPeriod, bool iqInverted, uint32_t timeout)
{
    ...
    case MODEM_FSK:
        ...
        RadioSetModem((SX126x.ModulationParams.PacketType == PACKET_TYPE_GFSK) ? MODEM_FSK : MODEM_LORA);
        SX126xSetModulationParams(&SX126x.ModulationParams);
        LOG_ERR("D. CrcMode %d, HeaderType %d, InvertIQ %d, PayloadLength %d, PreambleLength %d",
                SX126x.PacketParams.Params.LoRa.CrcMode,
                SX126x.PacketParams.Params.LoRa.HeaderType,
                SX126x.PacketParams.Params.LoRa.InvertIQ,
                SX126x.PacketParams.Params.LoRa.PayloadLength,
    ...
    case MODEM_LORA:
        ...
        LOG_ERR("C. CrcMode %d, HeaderType %d, InvertIQ %d, PayloadLength %d, PreambleLength %d",
                SX126x.PacketParams.Params.LoRa.CrcMode,
                SX126x.PacketParams.Params.LoRa.HeaderType,
                SX126x.PacketParams.Params.LoRa.InvertIQ,
                SX126x.PacketParams.Params.LoRa.PayloadLength,
...
void RadioSend(uint8_t *buffer, uint8_t size)
{
    LOG_HEXDUMP_DBG(buffer, size, "RadioSend");
    ...
    SX126xSetPacketParams(&SX126x.PacketParams);
    LOG_ERR("Setting Packet Params...");
    LOG_ERR("B. CrcMode %d, HeaderType %d, InvertIQ %d, PayloadLength %d, PreambleLength %d",
            SX126x.PacketParams.Params.LoRa.CrcMode,
            SX126x.PacketParams.Params.LoRa.HeaderType,
            SX126x.PacketParams.Params.LoRa.InvertIQ,
            SX126x.PacketParams.Params.LoRa.PayloadLength,
    ...
    LOG_ERR("Sending Payload");
    SX126xSendPayload(buffer, size, 0);
    LOG_ERR("Setting TxTimeout to %d", TxTimeout);
...
void RadioSetMaxPayloadLength(RadioModems_t modem, uint8_t max)
{
    if (modem == MODEM_LORA)
    {
        ...
        LOG_ERR("A. CrcMode %d, HeaderType %d, InvertIQ %d, PayloadLength %d, PreambleLength %d",
                SX126x.PacketParams.Params.LoRa.CrcMode,
                SX126x.PacketParams.Params.LoRa.HeaderType,
                SX126x.PacketParams.Params.LoRa.InvertIQ,
                SX126x.PacketParams.Params.LoRa.PayloadLength,
...
void RadioIrqProcess(void)
{
    ...
    if (isIrqFired == true)
    {
        ...
        {
            LOG_ERR("IRQ_PREAMBLE_DETECTED");
            //__NOP( );
        }

        if ((irqRegs & IRQ_SYNCWORD_VALID) == IRQ_SYNCWORD_VALID)
        {
            LOG_ERR("IRQ_SYNCWORD_VALID");
            //__NOP( );
        }

        if ((irqRegs & IRQ_HEADER_VALID) == IRQ_HEADER_VALID)
        {
            LOG_ERR("IRQ_HEADER_VALID");
            //__NOP( );
        }

        if ((irqRegs & IRQ_HEADER_ERROR) == IRQ_HEADER_ERROR)
        {
            LOG_ERR("IRQ_HEADER_ERROR");
            TimerStop(&RxTimeoutTimer);
            if (RxContinuous == false)
            {
                //!< Update operating mode state to a value lower than \ref MODE_STDBY_XOSC
